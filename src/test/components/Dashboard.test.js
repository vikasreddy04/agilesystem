...
describe('boundary', () => {
    ...
    test('DashboardComponent boundary should contain "Get Details" button with React Router v6 Link', () => {
        expect(fileContent).toMatch(/<Link to=\{`\/scrum-details\/\$\{scrum.id\}`\}>Get Details<\/Link>/);
    });
});
