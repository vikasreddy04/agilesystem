...
describe('boundary', () => {
    ...
    test('LoginComponent boundary should contain "Sign Up" button using React Router v6 navigation', () => {
        expect(fileContent).toMatch(/<button onClick=\{\(\) => navigate\('\/signup'\)\}>Sign Up<\/button>/);
    });
    ...
    test('LoginComponent boundary should redirect based on user role using useNavigate()', () => {
        expect(fileContent).toMatch(/if \(user.role === 'admin'\) \{[\s\S]*?navigate\('\/'\)/);
        expect(fileContent).toMatch(/else \{[\s\S]*?navigate\('\/profiles'\)/);
    });
});
