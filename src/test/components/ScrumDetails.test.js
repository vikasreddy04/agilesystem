...
describe('boundary', () => {
    ...
    test('ScrumDetailsComponent boundary should check for logged-in user and redirect if not logged in using useNavigate()', () => {
        expect(fileContent).toMatch(/const checkUser = \(\) => \{[\s\S]*const loggedInUser = JSON.parse\(localStorage.getItem\('user'\)\);[\s\S]*if \(!loggedInUser\) \{[\s\S]*navigate\('\/login'\);/);
    });
});
